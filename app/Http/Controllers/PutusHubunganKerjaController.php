...
class PutusHubunganKerjaController extends Controller
{
  public function putushubungankerja (Request $request, $id)
  {
    // dd($id);die;
    $user = auth()->user();
    $pegawai = $id;
  ...
  $sp_main_code='PHK'. '-'. date('d').'-'.date('m').'-'.date('Y').'-'."000".$kode+1;
   //code header
  ...
  { $kodes = $idss->id;}
    ...
    return view ('berita_acara.putushubungankerja', compact('user', 'pt', 'lokasi', 'tipe', 'employee', 'sp_main_code', 'tr_acc_sp_h_code', 'pegawainya'));
  }

  public function store_putushubungankerja(Request $request)
  {
