...
<table id="myTable">
    <thead>
        <tr>
            ...
            <th><input type="text" id="filterCity" onkeyup="filterTable()" placeholder="Cari Tracking Assesor"></th>
    ...
    <tbody>
        @foreach($employee as $user)
            <tr>
                {{--  <td>{{ $user->emp_name }}</td>  --}}
                <td data-table-header="Title"><a class="underlineHover" href="/asasmen/create_asasmen_basic/{{ $user->emp_name }}">{{ $user->emp_name }}</a> </td>
                <td>{{ $user->emp_subdivision }}</td>
                <td>{{ $user->Ms_record_asses }}</td>
