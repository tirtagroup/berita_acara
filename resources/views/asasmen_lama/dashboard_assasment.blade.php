...
<div class="container mt-2">
    ...
    <div class="row">
        ...
        <div class="col-md-12 card-header text-center font-weight-bold">
          <h2>Dashboard STNK</h2>
        </div>
        <a href="/stnk/ms_stnk"><button class="btn btn-success">Register</button></a>
        <div class="col-md-12">
            <table id="dataTable4" class="display" style="width:100%">
              <thead>
                <tr>
                  <th scope="col">No. Pol</th>
                  <th scope="col">No. STNK 1thn</th>
                  <th scope="col">Pemilik</th>
                  <th scope="col">Jenis</th>
                  <th scope="col">Masa Berlaku 1 Tahun</th>
                  <th scope="col">Lokasi Akhir</th>
                </tr>
              </thead>
              <tbody>
                @foreach ($data as $tipe)
                <tr>
                    <td data-table-header="Title"><a class="underlineHover" href="/stnk/detail_lokasi_stnk/{{ $tipe->nomor_polisi }}">{{ $tipe->nomor_polisi }}</a> </td>
                    <td>{{ $tipe->nomor_stnk_1_tahun}}</td>
                    <td>{{ $tipe->pemilik}}</td>
                    <td>{{ $tipe->jenis}}</td>
                    <td>{{ date_format(date_create($tipe->awal_masa_berlaku_1_tahun),"d/m/Y") }} - {{ date_format(date_create($tipe->akhir_masa_berlaku_1_tahun),"d/m/Y") }}</td>
                    <td>{{ $tipe->lokasi_akhir}}</td>
                </tr>
                @endforeach
              </tbody>
              <tfoot>
          ...
          <div class="modal-body">
            <form action="javascript:void(0)" id="addEditBookForm" name="addEditBookForm" class="form-horizontal" method="POST">
              ...
              <div class="form-group">
                <label for="name" class="col-sm-4 control-label">No. STNK</label>
                <div class="col-sm-12">
                  <input type="text" class="form-control" id="nomor_stnk" name="nomor_stnk" required>
                </div>
              </div>
              <div class="form-group">
...
<script type="text/javascript">
 $(document).ready(function($){
    ...
    $('#addNewBook').click(function () {
       ...
       $('#ajaxBookModel').html("Add Master STNK");
    ...
    $('body').on('click', '.edit', function () {
        ...
        $.ajax({
            ...
            success: function(res){
              $('#ajaxBookModel').html("Edit STNK");
              $('#ajax-book-model').modal('show');
              $('#id').val(res.id);
              $('#nomor_stnk').val(res.nomor_stnk);
