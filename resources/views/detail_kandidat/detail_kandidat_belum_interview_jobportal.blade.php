...
<img width="100" height="130" src="{{ asset($poto->file_path) }}">
  ...
  {{--  <form  action="/tr_candidates/Shortlist/{{$header['id']}}" method="post" enctype="multipart/form-data">  --}}
    <form  action="/tr_candidates/candidate_interview_jobportal/{{$header['id']}}" method="post" enctype="multipart/form-data">
  {{ csrf_field() }}
            <table>
...
<table>
    <tr>
      <td>
        <center> <label for="">Apakah Kandidat Akan Lanjut Proses Orientasi?</label>
            <select name = "status_interview" class="form-control" >
                <option value="lulus" style="weight:50px">Iya</option>
                <option value="tidak lulus" style="weight:50px">Tidak</option>
            </select> </center>
            <br>
            <center> <label for="">Alasan Di Terima / Tidak Di terima</label>
                <input type="text" name="alasan_interview" id="" class="form-control" required> </center>
                <br>
