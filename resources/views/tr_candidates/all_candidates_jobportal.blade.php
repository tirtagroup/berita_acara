...
@foreach($call as $p)
@endforeach
<form action="{{ route('all_candidates') }}" method="POST">
{{--  <form  action="/tr_candidates/all_candidates/{{$p->Email }}" method="post" enctype="multipart/form-data">  --}}
  @csrf
<title>
    All Candidates
  </title>
...
<body>
<div class="container">
  <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Forms/</span> Semua Kandidat</h4>
          <div class="dropdown">
            <!--<button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">-->
            <!--  Filter-->
            <!--</button>-->
             <button class="button" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Filter
            </button>
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
              ...
              <a class="dropdown-item" href="/tr_candidates/kandidat_belum_interview">- Belum Di Interview</a>
          ...
          <a href="/isi_kandidat" class="button">Isi Kandidat</a>
          <a href="/tr_candidates/all_candidates" class="button">Cv Software</a>
        </div>
            ...
            <table class="table" id="dataTable">
                ...
                <thead>
                  <tr >
                    ...
                    <th scope="col" >Tgl. Apply</th>
                    <th scope="col" >PIC Short</th>
              ...
              <thead>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{ $trCandidate->CekCV }}</td>
                    <td data-table-header="Title">
                      {{-- @if($trCandidate->CekCV == 'Erika')
                        <a class="underlineHover" href="/tr_candidates/edit_from_jobportal/{{ $trCandidate->Email }}">{{ $trCandidate->Name }}</a>
                      @else --}}
                        <a class="underlineHover" href="/detail_kandidat/detail_kandidat_progress_jobportal/{{ $trCandidate->Ktp }}">{{ $trCandidate->Name }}</a>
                      {{-- @endif --}}
                    </td>
                    <td>{{ $trCandidate->domisili }}</td>
                    <td>{{ $trCandidate->umur }}</td>
                    <td>{{ $trCandidate->jenis_kelamin }}</td>
                    <!--<td>{{ $trCandidate->Email }}</td>-->
                    <td>{{ $trCandidate->Position_aplly1 }}</td>
                    <td>{{ date_format(date_create($trCandidate->created_at),"d/m/Y") }}</td>
                    <td>{{ $trCandidate->PICShortlist }}</td>
                    <td>
                      {{--  {{ $trCandidate->status_shortlist }}  --}}
                      {{--  {{ $trCandidate->status_shortlist }}  --}}
                      {{--  <select class="form-control" name="Location_Code" >
                        ...
                        {{--  <select name="status_shortlist" class="form-control">
                          @foreach($master_short as $ms)
                              <option value="{{ $ms->status_desc }}">{{ $ms->status_desc }}</option>
                          @endforeach
                        </select>  --}}

                        @if($trCandidate->ms_short_status == 'Shortlisted')
                        <button type="button" class="btn btn-success" data-toggle="popover" title="Popover title" >
                          Shortlisted</button>
                        @elseif ($trCandidate->ms_short_status == 'Called')
                        <button type="button" class="btn btn-primary" data-toggle="popover" title="Popover title" >
                        Called</button>
                        @elseif ($trCandidate->ms_short_status == 'Interview')
                        <button type="button" class="btn btn-primary" data-toggle="popover" title="Popover title" >
                          Interview</button>
                        @else
                    ...
                    <!--<td>-->
                    <!--   <a href="/edit_status/{{ $trCandidate->Email }}" class="btn btn-warning edit">&nbsp; Edit  &nbsp;</a>-->
                    <!--</td>-->
...
<style>
  ...
  border: 2px solid #dad5d0; /* Garis tepi tombol */
